...
# Optional: use SSL dataset for two-view alignment/uniformity on CIFAR
try:
    from eval.datasets.cifar100_multicrop import CIFAR100MultiCrop, MultiCropCfg
except Exception:
    CIFAR100MultiCrop = None
    MultiCropCfg = None
...
def make_eval_loaders(
    ...
    if dataset == "cifar100":
        ...
        train_ds = datasets.CIFAR100(root=data_root, train=True, download=True, transform=tfm)
        test_ds = datasets.CIFAR100(root=data_root, train=False, download=True, transform=tfm)
        num_classes = 100

    elif dataset in ("imagenet100", "inet100", "imagenet-100"):
        tfm = transforms.Compose(
            [
        ...
        test_ds = datasets.ImageFolder(val_dir, transform=tfm)
        num_classes = len(train_ds.classes)

    else:
        raise ValueError(f"Unknown dataset '{dataset}'. Use cifar100 or imagenet100.")
...
def make_two_view_loader(
    ...
    if dataset == "cifar100":
        if CIFAR100MultiCrop is None or MultiCropCfg is None:
            raise RuntimeError("CIFAR100MultiCrop not importable. Cannot do 2-view diagnostics on CIFAR100.")
        mc_cfg = MultiCropCfg(num_global_views=2, num_local_views=0, out_size=32)
        ds = CIFAR100MultiCrop(root=data_root, train=True, cfg=mc_cfg, download=True)
    ...
    if dataset in ("imagenet100", "inet100", "imagenet-100"):
        ...
        train_dir = os.path.join(data_root, "train")
        ds = TwoViewImageFolder(train_dir, transform=aug)
        if max_items is not None and len(ds) > max_items:
            idx = torch.randperm(len(ds))[:max_items].tolist()
            ds = torch.utils.data.Subset(ds, idx)
...
def main() -> None:
    ...
    ap.add_argument("--dataset", type=str, default="cifar100", help="cifar100 or imagenet100")
    ...
    def run_eval(coord_label: str, transform_fn, title_suffix: str, file_suffix: str, *, qp_variant: str):
        ...
        if args.do_align_uniform:
            two_view_loader = make_two_view_loader(
                ...
                batch_size=args.eval_bs,
                num_workers=args.num_workers,
                image_size=args.image_size if args.dataset.lower() != "cifar100" else 32,
