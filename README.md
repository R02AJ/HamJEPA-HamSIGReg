...
```bash
bash scripts/run_train_cifar_baseline.sh
bash scripts/run_train_cifar_hjepa_mv.sh
```
...
python scripts/train_imagenet_hamjepa.py --config configs/imagenet_hjepa_mv.yaml
```

Wrapper scripts:

```bash
bash scripts/run_train_imagenet_sigreg_tokens.sh
bash scripts/run_train_imagenet_hjepa_mv.sh
```
...
python scripts/eval_suite.py \
  --config configs/cifar100_sigreg_tokens.yaml \
  --ckpt checkpoints/cifar100_sigreg_tokens.pth \
  --out eval_runs/cifar100_sigreg_tokens \
  --dataset cifar100 \
  --data_root ./data
```
...
```bash
bash scripts/run_eval_cifar.sh checkpoints/cifar100_sigreg_tokens.pth configs/cifar100_sigreg_tokens.yaml
bash scripts/run_eval_cifar_all.sh
bash scripts/run_eval_cifar_hjepa_mv.sh
```

### ImageNet

```bash
python scripts/eval_suite_imagenet.py \
  --config configs/imagenet_sigreg_tokens.yaml \
  --ckpt checkpoints/imagenet_sigreg_tokens.pth \
  --out eval_runs/imagenet_sigreg_tokens \
  --dataset imagenet \
...
```bash
bash scripts/run_eval_imagenet_sigreg_tokens.sh
bash scripts/run_eval_imagenet_hjepa_mv.sh
```
